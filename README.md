...
這份專案將展示如何使用 `Python` 來實現一個 `Pixiv` 爬蟲，以便從該網站獲取插畫圖片。`Pixiv` 網站採用了大量的 `AJAX` 技術，如果您對 `AJAX` 的概念還不太熟悉，不用擔心，我們將會在後續的解釋中進一步闡述。
...
雖然程式比較簡陋且沒有做成 `GUI` 或 `EXE` 檔等等，但這也是學習過程中的一部分，未來會考慮進一步改進這個爬蟲程式，讓它更加完善和便利。
